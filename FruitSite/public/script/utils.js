var panelNormalClass = "panel";
var panelColClass="panelcollapsed";
var panelHead="h2";
var panelContentCls="panelcontent";
...
function setUpPanels()
{
	...
	// get all headings
	var headingTags = document.getElementsByTagName(panelHead);
	...
	for (var i=0; i<headingTags.length; i++)
	{
		...
		if (el.parentNode.className != panelNormalClass && el.parentNode.className != panelColClass)
			continue;
		...
		if (panelsStatus[name] == "false")
			el.parentNode.className = panelColClass;
		else
		if (panelsStatus[name] == "true")
			el.parentNode.className = panelNormalClass;
		else
		{
			// if no saved setting, see the initial setting
			panelsStatus[name] = (el.parentNode.className == panelNormalClass) ? "true" : "false";
		}
		...
		el.onclick = function() 
		{
			...
			var collapsed = (target.className == panelColClass);
...
function animateTogglePanel(panel, expanding)
{
	...
	{
		if (elements[i].className == panelContentCls)
		{
...
function animateStep(panelContent, iteration, stepHeight, direction)
{
	...
	else
	{
		...
		panelContent.parentNode.className = (direction<0) ? panelColClass : panelNormalClass;
		// clear inline styles
